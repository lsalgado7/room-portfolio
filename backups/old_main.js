...
// list of keywords to identify non-interactable meshes
...
loader.load("/models/Room_Portfolio.glb", (glb)=>{
  glb.scene.traverse(child=>{
    if(child.isMesh){
      ...
      // get interactable objects and add them to raycasterObjects
...
// UPDATE need to make it so you can pass values to change the animation
...
const render = () =>{
  ...
  if(currentIntersects.length>0){
    ...
    if(currentIntersectObject.name.includes("click")) {
      // I only want hover for clickables
      // work/abt/contact diff anim, separate from git/linkd
      // all other anim are on click
